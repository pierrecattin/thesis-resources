...
freq.vs.accuracy.multiplot <- function (y.train, train.probs,
  ...
  plots <- list(freq.vs.accuracy.plot(y.train, train.probs, conf, "Training", ytitle=T, legend=F, granularity),
     freq.vs.accuracy.plot(y.dev, dev.probs, conf, "Dev", ytitle=F, legend=F, granularity),
     freq.vs.accuracy.plot(y.test, test.probs, conf, "Testing", ytitle=F, legend=T, granularity))
